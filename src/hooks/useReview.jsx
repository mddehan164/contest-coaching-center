...
export const useReviews = () => {
  ...
  const handleSetSelectedReview = (data) =>
    dispatch(setSelectedReviewData(data));
  ...
  const handleOpenConfirmationModal = () =>
    dispatch(setReviewConfirmationModal(true));
    ...
    addReview({ data: formData })
      ...
      .then((response) => {
        if (response?.success) {
          ...
          console.log("res from add", response);
          successNotify(response?.message);
        }
      })
      .catch((err) => {
