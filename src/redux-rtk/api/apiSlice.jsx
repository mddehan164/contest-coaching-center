...
const getCsrfToken = async () => {
  try {
    await fetch(`${import.meta.env.VITE_BASE_URL || "http://localhost:8000"}/sanctum/csrf-cookie`, {
      credentials: 'include'
    })
  } catch (error) {
...
const baseQueryWithReauth = async (args, api, extraOptions) => {
  ...
  if (result?.error?.code === 'ERR_NETWORK' || result?.error?.status === 'FETCH_ERROR') {
    console.warn('Backend server not available')
    
    return {
      error: {
        status: 'CUSTOM_ERROR',
        data: { message: 'Backend server not available. Please start the backend server or use mock mode.' }
      }
    }
  }
