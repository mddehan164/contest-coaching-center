...
// *view data from paginated list
export const viewDataFromPaginatedList = ({ idToView, meta, data }) => {
  const { pageSize, totalPages, currentPage } = meta;
  const updatedData = {};
  let updatedDataList = [];
  let items = [];
  ...
  // View the item
  items = items.filter((item) => item._id !== idToView);
...
export const updateDataInDataList = ({
  dataList,
  updatedItem,
  matchKey = "id",
}) => {
...
export const updateDataInPaginatedPages = ({
  data,
  meta,
  updatedItem,
  matchKey = "id",
