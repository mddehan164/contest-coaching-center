...
export default function ViewDetails({ data, isOpen, onClose, title = "View Details", fieldsToShow = null }) {
  ...
  const formatValue = (value, key) => {
    ...
    // Handle images
    if ((key.toLowerCase().includes('img') || key.toLowerCase().includes('image') || key.toLowerCase().includes('photo')) && typeof value === 'string' && value.startsWith('http')) {
      return (
        <div className="flex flex-col gap-3">
          ...
          <div className="text-center space-y-2">
            ...
            <div className="text-xs text-gray-600 break-all font-mono bg-gray-100 p-2 rounded">
              <span className="font-semibold">Source: </span>{value}
            </div>
          </div>
        </div>
      );
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            {value.map((item, index) => (
              <div key={index} className="text-sm bg-white border border-gray-200 px-3 py-2 rounded-md">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          <div className="space-y-3">
            {Object.entries(value).map(([nestedKey, nestedValue]) => (
              <div key={nestedKey} className="border-b border-gray-100 pb-2 last:border-b-0 last:pb-0">
    ...
    if (typeof value === 'string' && value.length > 100) {
      return (
        <div className="space-y-2">
          <div className="bg-white border border-gray-200 rounded-lg p-3">
            <p className="text-sm leading-relaxed whitespace-pre-wrap break-words">{value}</p>
          </div>
    ...
    if (typeof value === 'number') {
      ...
      if (key.toLowerCase().includes('price') || key.toLowerCase().includes('cost') || key.toLowerCase().includes('amount') || key.toLowerCase().includes('fee')) {
        return (
      ...
      >
        ...
        >
          ...
          <div className="flex justify-between items-center mb-4 sm:mb-6 border-b pb-3 sm:pb-4">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 flex-1 pr-4">{title}</h2>
            <button
              onClick={onClose}
          ...
          <div className="max-h-[75vh] overflow-y-auto overflow-x-hidden message-scrollbar">
            {dataEntries.length > 0 ? (
              ...
              <div className="text-center py-12">
                <div className="text-gray-400 text-lg mb-2">No fields to display</div>
                <div className="text-gray-500 text-sm">Please specify which fields to show using the fieldsToShow prop</div>
              </div>
            )}
          </div>
