...
const SummaryEditor = () => {
  ...
  return (
    <div className="p-4 space-y-8 xl:px-44 xl:text-xl">
      <h2 className="text-2xl font-semibold text-center xl:text-4xl">Edit Summary</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-2">
          ...
          <input
            ...
            className="block w-full p-2 border rounded"
          />
          <button onClick={handleSaveChanges} className="bg-black text-white px-4 py-2 rounded">
            Save Changes
          </button>
        </div>
        ...
        {isEditing && (
          <div className="bg-gray-100 p-4 rounded-md shadow-md w-full max-w-xs mx-auto">
            ...
            <QuickStat data={{ ...newSummary, icon: summaryData[editIndex]?.icon }} />
          </div>
        )}
      </div>
      ...
      <div className="text-center mt-6">
        <button className="px-6 py-2 rounded bg-black text-white cursor-pointer" onClick={()=>{alert("Are you sure wanna change?")}}>Save All Changes</button>
      </div>
    </div>
  );
};
