...
const CourseEditor = () => {
  ...
  const handleAddOrEditCourse = () => {
    const newBody = newCourse.shortDesc
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean);
    const course = {
      ...
      <section className="bg-white rounded shadow p-4">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 space-y-1">
            <h3 className={`text-xl font-bold ${hero.color}`}>
              {hero.data.title}
            </h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded max-w-md w-full space-y-4">
            ...
            <textarea
              ...
              value={formHero.des}
              onChange={(e) =>
                setFormHero({ ...formHero, des: e.target.value })
              }
            />
            <div>
              {formHero.image && (
