...
const GalleryEditor = () => {
  ...
  const categories = ["success", "campus", "student", "others"];
  ...
  const handleFileSelect = (e) => {
    ...
    if (validFiles.length !== files.length) {
      errorNotify(
        "Some files were skipped. Only images under 5MB (jpg, png, webp, etc.) are allowed."
      );
    }
  ...
  const toggleImageStatus = async (imageId, currentStatus) => {
    try {
      ...
      if (response.data.success) {
        ...
        await fetchGalleryImages();
        successNotify(
          `Image status changed to ${response.data.data.status_text}`
        );
      }
    } catch (error) {
  ...
  const openModal = (imageSrc, e) => {
    if (e && (e.target.type === "checkbox" || e.target.closest("button")))
      return;
  ...
  return (
    <div className="w-full mx-auto p-4 xl:px-44">
      ...
      <div className="flex justify-center space-x-4 mb-6">
        {["All", "Success", "Campus", "Student", "Others"].map((cat) => (
          <button
            key={cat}
