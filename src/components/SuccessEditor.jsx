...
const SuccessEditor = () => {
  ...
  const handleAddOrEdit = () => {
    ...
    setNewPerson({ img: '', title: '', subtitle: '', rank: '', description: '' });
    setPreviewImage('');
  };
    ...
    <div className="p-4 space-y-8 xl:px-44 xl:text-xl">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="space-y-2 col-span-2">
          <input type="file" name="img" onChange={handleInputChange} className="block w-full" />
          <input name="title" value={newPerson.title} onChange={handleInputChange} placeholder="Name" className="block w-full p-2 border rounded" />
          <input name="subtitle" value={newPerson.subtitle} onChange={handleInputChange} placeholder="Department/Year" className="block w-full p-2 border rounded" />
          <input name="rank" value={newPerson.rank} onChange={handleInputChange} placeholder="Rank" className="block w-full p-2 border rounded" />
          <textarea name="description" value={newPerson.description} onChange={handleInputChange} placeholder="Success Story" className="block w-full p-2 border rounded h-24" />
        ...
        {people.map((person, index) => (
          <div key={index} className="border rounded shadow p-2 relative group bg-white">
            <div className='w-full flex items-center justify-center aspect-square'>
                <img src={person.img} alt={person.title} className="w-[80%] object-cover rounded-full" />
            </div>
            ...
            <div className="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 flex flex-col justify-center items-center space-y-2 transition">
              <button onClick={() => setPreviewFull(person)} className="bg-white text-black px-3 py-1 rounded flex items-center gap-1"><FaEye /> Preview</button>
      ...
      {previewFull && (
        <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex justify-center items-center">
          <div className="bg-white rounded-lg p-4 w-[90%] md:w-[70%] lg:w-[50%] relative flex flex-col gap-2 items-center">
            ...
            <p className="text-sm mt-2 whitespace-pre-line">{previewFull.description}</p>
          </div>
        </div>
      )}
    </div>
