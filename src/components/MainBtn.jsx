...
const MainBtn = ({ data, btnStyle, isActive, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`max-sm:py-1 max-sm:px-2 max-sm:rounded-md text-sm sm:py-2 sm:px-4 px-6 py-4 lg:px-3 lg:py-2 md:px-2 md:py-1 rounded-lg ${textColor} bg-${bgColor} ${hoverColor} ${fullWidth}`}
    >
      {data}
    </div>
  );
