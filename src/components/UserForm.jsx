...
const UserForm = ({data, handleChange, handleSubmit}) => {
  return (
    <form onSubmit={handleSubmit} className='w-full sm:w-1/2 xl:w-[55%] 2xl:w-1/3 h-full sm:px-5 md:px-7 lg:px-10 xl:px-14 xl:py-10 px-2 py-5 bg-contestLight rounded-md shadow-md border-none'>
      ...
      {
        data.fields.map((field, idx) => (<div key={idx}><input onChange={handleChange} placeholder={field.placeholder} type={field.type} name={field.name} className='py-3 rounded-md mt-5 w-full px-2 text-headerColorHover outline-none shadow-sm'/> <br/></div>))
      }
