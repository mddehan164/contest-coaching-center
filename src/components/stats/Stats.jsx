...
const Stats = ({ stat }) => {
  const gradient = Array.isArray(stat.backgroundGradient) ? stat.backgroundGradient : [];
  ...
  return (
    <div className="h-auto p-4 bg-white hover:shadow-xl shadow-md cursor-pointer border-t-4 border-headerColor rounded-md">
      <div className="flex flex-col items-center justify-center gap-4">
        <div
          className={`rounded-full w-40 h-40 md:w-48 md:h-48 lg:w-36 lg:h-36 ${!stat.img ? 'flex items-center justify-center flex-col text-white font-semibold gap-2' : ''}`}
          style={!stat.img ? gradientStyle : {}}
        >
          {
            !stat.img ? React.createElement(stat.icon, { className: 'text-4xl md:text-5xl lg:text-6xl' }) :
